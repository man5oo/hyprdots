...
# script=`basename "$0"`
script='wofi_iwd.sh "$0"'
# pathname=`dirname "$0"`
pathname='/home/manuel/.config/wofi "$0"'
configfile='/home/manuel/.config/wofi/configwifi'
...
function wofi() {
    command wofi -d -I --conf configfile "$@"
}
function rofi() {
...
case $script in
    ...
    wofi_*)
        label_interface=""
        menu_interface="wofi -l 0 -p $label_interface"
        label_ssid=""
